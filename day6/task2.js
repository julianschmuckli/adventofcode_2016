...
var characters = [],solution_code="";

for(var i=0;i<columns;i++){
	characters=[];
	for(var ia=0;ia<rows;ia++){
		addChar(data[ia][i]);
	}
	solution_code+=leastCommonCharacter();
}
...
function addChar(chr){
	...
	if(characters[chr]==undefined){
		...
		characters[chr]=characters[chr]+1;
	}
}
function leastCommonCharacter(){
	var max_value=999999,max_key;
	for(var i=0;i<characters.length;i++){
		max_value = characters[i];
		
	}
	for (key in characters){
		if(characters[key]<max_value){
